...
public class OpeningDaySpecials implements PricingRules {
    ...
    public double applyRules(List<Item> scannedItems) {
        ...
        for (Item item : scannedItems){
            if(item.getSku().trim().equals("vga")){
                vgaCount +=1; //just count them, price will be calcualted later
                vgaPrice = item.getPrice();
            }
            else if(item.getSku().trim().equalsIgnoreCase("atv")){
                total += item.getPrice();
                atvPrice = item.getPrice();
                atvCount +=1;   
            }
            else if(item.getSku().trim().equals("ipd")){
                total += item.getPrice();
                ipdCount +=1;
            }
            else if(item.getSku().trim().equals("mbp")){
                total += item.getPrice();
                mbpCount +=1;
            }
        }
        System.out.println("Expected Price:"+total);
        if(ipdCount >4){
